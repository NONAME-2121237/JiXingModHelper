...
左边下拉选类型，贴图还能再点细分类（手牌、立绘、筹码、地图格子…），中间点一项右边就出预览。看中了直接「去制作替换」，或先「导出 PNG」留着。

![浏览资源](docs/screenshots/02_browse.png)

> 小提示：**筹码**在资源名里可能写作 Relic，但界面就叫筹码；走路/攻击那种一帧帧的图在**角色动作帧**分类里。文本/动画列表如果空的，先点一次**刷新索引**（第一次要扫全部包，几千个，得等一会儿）。
...
| 任务栏是 Python 图标 | 用 `JiXingModHelper.exe` 启动（不是 pythonw）；固定过的先取消固定再开 |
